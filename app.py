...
if uploaded_file is not None:
    ...
    st.title('Welcome to Chat Insights')
    if st.sidebar.button("Show Analysis"):
        st.title('Showing Insights for '+ selected_user)
        ...
        with col1:
            st.header("Total Messages",divider='red')
            st.title(num_messages)

        with col2:
            st.header("Total No.of Words",divider='red')
            st.title(num_words)

        with col3:
            st.header("Media Shared",divider='red')
            st.title(media_omitted)

        with col4:
            st.header("Total Links Shared",divider='red')
            st.title(links)
        ...
        if selected_user == 'Overall':
            ...
            with col1:
                ax.bar(busycount.index, busycount.values, color='orange')
        ...
        fig, ax = plt.subplots()
        ax.plot(time['Time'], time['Message'], color='red')
        plt.xticks(rotation='vertical')
        plt.tight_layout()
        st.pyplot(fig)
        ...
        with col1:
            ...
            st.header("Most Busy Day", divider='red')
        ...
        with col2:
            ...
            st.header("Most Busy Month" ,divider='red')
        ...
        st.subheader('Sentiment: ', divider='red')
        st.write(get_sentiment(stats.get_all_messages(selected_user, df)))

        st.subheader('Summary: ', divider='rainbow')
        st.write(get_summary(stats.get_all_messages(selected_user, df)))
